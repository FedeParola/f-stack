...
void print_msg(char *data, int size){
...
int loop(void *arg)
{
    ...
    for (i = 0; i < nevents; ++i) {
        ...
        if (event.flags & EV_EOF) {
            ...
            printf("Contents %s\n", data);
            memcpy(data, html_template, strlen(html_template));
            rte_mb->data_len = strlen(html_template);
            rte_mb->pkt_len = rte_mb->data_len;
            printf("rte_mb->data_len %d\n", rte_mb->data_len);
            mb = ff_mbuf_get(NULL, rte_mb, data, rte_mb->data_len);
            printf("mb points to %p\n", mb);
